...
def generate_images(csv_file, image_folder, sae_model, min_max_scaler):
	...
	for user in tqdm(users):
		...
		for date in dates:
			...
			cur_row = np.array(user_data[user_data['date'] == cur_date].drop(["user", "day", "date", "attack"], axis=1))
			
			if not pd.isnull(prev_prev_date):
				prev_prev_row = np.array(user_data[user_data['date'] == prev_prev_date].drop(["user", "day", "date", "attack"], axis=1))
			else:
				continue

			if not pd.isnull(prev_date):
				prev_row = np.array(user_data[user_data['date'] == prev_date].drop(["user", "day", "date", "attack"], axis=1))
			else:
				continue
...
def main():
	...
	generate_images("dev.csv", "DevImages", sae_model, min_max_scaler)
